...
async function loadLevel(index) {
  ...
  try {
    ...
    gs.showingNarrative = true;
    gs.narrativeTimer = 2.5;

    stateManager.loadLevel(index);
    analytics.levelStart(index);
    // Start music for this tank theme
...
function update(dt) {
  ...
  if (state === States.MAIN_MENU) {
    ...
    const menuItems = [];
    menuItems.push(hasSave ? 'continue' : 'start');
    if (hasSave) menuItems.push('new_game');
    if (hasCompleted) menuItems.push('level_select');
    menuItems.push('settings');

    if (dir.y < -0.5) gs.menuSelection = Math.max(0, gs.menuSelection - 1);
    if (dir.y > 0.5) gs.menuSelection = Math.min(menuItems.length - 1, gs.menuSelection + 1);
    ...
    if (input.isConfirmPressed()) {
      ...
      } else if (selected === 'level_select') {
        gs.levelSelectActive = true;
        gs.levelSelectIndex = 0;
      } else if (selected === 'settings') {
        stateManager.transition(States.SETTINGS, { from: 'main_menu' });
      }
    }
    input.flush();
    return;
  ...
  // 2. Physics
  const drainRate = difficulty.getEnergyDrainRate();
  if (isMoving) {
    momentum.applyInput(dir, dt);
    energy.drainEnergy(drainRate * dt);
  }
  // Apply currents
  if (config.pumps) {
  ...
  for (const n of bounds.normals) momentum.resolveCollision(n);
  ...
  // 8. Energy grace period
  if (energy.updateGracePeriod(dt)) {
    stateManager.transition(States.GAME_OVER, { cause: 'energy' });
    analytics.gameOver(config.levelIndex, 'energy');
  }

  // 9. Stress
  if (config.stressEnabled) {
    stress.update(dt, config.stressRate);
...
function render(alpha) {
  ...
  if (state === States.MAIN_MENU) {
    ...
    if (gs.levelSelectActive) {
      ...
      ctx.fillText(strings.get('menu.level_select'), cw / 2, ch / 2 - 80);
      const saveData = save.load();
      const completed = saveData ? saveData.completedLevels : [];
      const starRatings = saveData ? (saveData.starRatings || {}) : {};
      const totalW = LEVEL_PATHS.length * 80;
      const startX = cw / 2 - totalW / 2;
      for (let i = 0; i < LEVEL_PATHS.length; i++) {
        const x = startX + i * 80;
        const y = ch / 2 - 30;
        const unlocked = completed.includes(i);
        const selected = i === gs.levelSelectIndex;
        ctx.fillStyle = unlocked ? (selected ? '#ffcc00' : '#22cc44') : '#444';
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`${i + 1}`, x + 32, y + 28);
        if (unlocked && starRatings[i]) {
          ctx.fillStyle = '#ffcc00';
          ctx.font = '13px sans-serif';
          ctx.fillText('★'.repeat(starRatings[i]) + '☆'.repeat(3 - starRatings[i]), x + 32, y + 50);
        }
        if (!unlocked) {
          ctx.fillStyle = '#888';
          ctx.font = '18px sans-serif';
          ctx.fillText('🔒', x + 32, y + 42);
        }
      }
      ctx.fillStyle = '#888';
      ctx.font = '13px sans-serif';
      ctx.fillText('←→ Select   Enter Play   Esc Back', cw / 2, ch / 2 + 70);
      return;
    }
    ...
    if (gs.confirmingNewGame) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText('🐟 Fish Food', cw / 2, ch / 2 - 80);
      ctx.fillStyle = '#fff';
      ctx.font = '20px sans-serif';
      ctx.fillText('Start a new game?', cw / 2, ch / 2 - 10);
      ctx.fillStyle = '#ff8844';
      ctx.font = '16px sans-serif';
      ctx.fillText('This will erase your current save data.', cw / 2, ch / 2 + 20);
      ctx.fillStyle = '#ffcc00';
      ctx.font = 'bold 18px sans-serif';
      ctx.fillText('Enter to confirm   Esc to cancel', cw / 2, ch / 2 + 60);
      return;
    }
    ...
    const hasCompleted = saveData && saveData.completedLevels && saveData.completedLevels.length > 0;
    const menuLabels = [];
    menuLabels.push(hasSave ? strings.get('menu.continue') : strings.get('menu.start'));
    if (hasSave) menuLabels.push(strings.get('menu.new_game'));
    if (hasCompleted) menuLabels.push(strings.get('menu.level_select'));
    menuLabels.push(strings.get('menu.settings'));

    for (let i = 0; i < menuLabels.length; i++) {
      const selected = i === gs.menuSelection;
      ctx.fillStyle = selected ? '#ffcc00' : '#ccc';
      ctx.font = selected ? 'bold 22px sans-serif' : '20px sans-serif';
      ctx.fillText(menuLabels[i], cw / 2, ch / 2 - 10 + i * 36);
    }
    ctx.fillStyle = '#666';
    ctx.font = '13px sans-serif';
    ctx.fillText('↑↓ Select   Enter/Space Confirm', cw / 2, ch / 2 - 10 + menuLabels.length * 36 + 20);
    // Version
    ctx.fillStyle = '#444';
    ctx.font = '11px sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('v0.1.0-mvp', cw - 12, ch - 10);
    return;
  }
  ...
  if (state === States.PROGRESS_MAP) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(allComplete ? 'Game Complete!' : 'Progress Map', cw / 2, 50);
    ...
    if (allComplete) {
      ...
      ctx.fillText(`Total Stars: ${'★'.repeat(totalStars)}${'☆'.repeat(maxStars - totalStars)}`, cw / 2, 85);
      ctx.fillStyle = '#aaa';
      ctx.font = '16px sans-serif';
      ctx.fillText(`${totalStars} / ${maxStars}`, cw / 2, 110);
    }

    // Level grid
    const totalW = levelCount * 80;
    const startX = cw / 2 - totalW / 2;
    for (let i = 0; i < levelCount; i++) {
      ...
      ctx.fillStyle = isCompleted ? '#22cc44' : isCurrent ? '#ffcc00' : '#444';
      ctx.fillRect(x, y, 65, 65);
      ...
      ctx.textAlign = 'center';
      ctx.fillText(`${i + 1}`, x + 32, y + 28);
      if (isCompleted && starRatings[i]) {
        ctx.fillStyle = '#ffcc00';
        ctx.font = '13px sans-serif';
        ctx.fillText('★'.repeat(starRatings[i]) + '☆'.repeat(3 - starRatings[i]), x + 32, y + 52);
      }
      if (isCurrent && gs.levelConfig) {
        ctx.fillStyle = '#aaa';
        ctx.font = '12px sans-serif';
        ctx.fillText(gs.levelConfig.narrativeDescription || '', cw / 2, ch / 2 + 60);
      }
    }
    ctx.fillStyle = '#888';
    ctx.font = '14px sans-serif';
    ctx.fillText('Press any key to continue', cw / 2, ch / 2 + 90);
    return;
  }
  ...
  if (state === States.PAUSED) {
    ...
    hud.render(ctx, {
      ...
      stressEnabled: gs.levelConfig?.stressEnabled,
  ...
  if (state === States.PLAYING) {
    ...
    // Narrative overlay
    if (gs.showingNarrative && gs.levelConfig) {
      ctx.fillStyle = 'rgba(0,0,0,0.6)';
      ctx.fillRect(0, 0, cw, ch);
      ctx.fillStyle = '#88ccff';
      ctx.font = 'bold 24px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(gs.levelConfig.narrativeDescription || '', cw / 2, ch / 2);
      ctx.fillStyle = '#666';
      ctx.font = '13px sans-serif';
      ctx.fillText('Press any key', cw / 2, ch / 2 + 40);
    }
